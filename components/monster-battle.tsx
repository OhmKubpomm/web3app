...
export default function MonsterBattle({ area, onAttack, damage, autoDamage }: MonsterBattleProps) {
  ...
  const generateMonster = () => {
    const monsters = {
      ป่า: [
        { name: "สไลม์", maxHealth: 10, image: "/placeholder.svg?height=200&width=200" },
        { name: "หมาป่า", maxHealth: 15, image: "/placeholder.svg?height=200&width=200" },
      ],
      ถ้ำ: [
        { name: "ค้างคาวยักษ์", maxHealth: 25, image: "/placeholder.svg?height=200&width=200" },
        { name: "โกเล็ม", maxHealth: 40, image: "/placeholder.svg?height=200&width=200" },
      ],
      วิหารโบราณ: [
        { name: "มัมมี่", maxHealth: 60, image: "/placeholder.svg?height=200&width=200" },
        { name: "ปีศาจโบราณ", maxHealth: 100, image: "/placeholder.svg?height=200&width=200" },
      ],
    }
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="relative w-64 h-64 mb-6">
        <motion.div
          animate={isAttacking ? { scale: 0.95, opacity: 0.8 } : { scale: 1, opacity: 1 }}
          className="relative"
        >
          <Image
          ...
          <AnimatePresence>
            {showDamage && (
              <motion.div
                ...
                animate={{ opacity: 1, y: damagePosition.y - 20, x: damagePosition.x }}
                exit={{ opacity: 0 }}
      ...
      <div className="flex gap-4">
        <Button size="lg" className="bg-red-600 hover:bg-red-700 text-white px-8" onClick={() => handleAttack()}>
          <Sword className="mr-2 h-5 w-5" />
          โจมตี
        </Button>

        {autoDamage > 0 && (
          <Button variant="outline" size="lg" className="border-yellow-500 text-yellow-400">
