...
import { Sword, Zap, Coins, Sparkles, Trophy, Calendar } from "lucide-react";
  ...
  if (!battleLog || battleLog.length === 0) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle>
            {locale === "th" ? "บันทึกการต่อสู้" : "Battle Log"}
          </CardTitle>
          <CardDescription>
            {locale === "th"
        ...
        <CardContent className="min-h-[300px] flex items-center justify-center">
          <div className="text-center text-gray-500">
            ...
            <p>
              {locale === "th"
                ? "ออกไปต่อสู้กับสัตว์ประหลาดเพื่อเพิ่มบันทึกการต่อสู้"
                : "Go fight some monsters to add battle records"}
            </p>
          </div>
        </CardContent>
      </Card>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      ...
                      <Badge
                        variant={
                          log.isCriticalHit ? "destructive" : "secondary"
                        }
                        className="text-xs"
                      >
                        {log.isCriticalHit
                    ...
                    {log.comboCount > 1 && (
                      <div className="mt-1 text-xs flex items-center">
                        ...
                        <span className="text-amber-500 dark:text-amber-400 font-medium">
                          {locale === "th"
                            ? `คอมโบ x${log.comboCount}!`
                            : `Combo x${log.comboCount}!`}
                        </span>
                      </div>
                    )}
                  </div>
