...
import { Shield, Package, Search, X, ChevronRight, ChevronLeft } from "lucide-react"
    ...
    <Card className="border-0 bg-transparent">
      ...
      <CardContent className="p-6">
        <AnimatePresence mode="wait">
          {selectedItem ? (
            <motion.div key="item-details" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="relative bg-black/40 border border-purple-500/50 rounded-lg p-6 mb-4">
                <Button variant="ghost" size="icon" className="absolute top-2 right-2" onClick={handleCloseDetails}>
                  <X className="h-4 w-4" />
                </Button>
                ...
                <div className="flex flex-col items-center mb-4">
                  <div className="relative mb-3">
                    ...
                    />
                    <Badge className={`absolute -top-2 -right-2 ${getRarityColor(selectedItem.rarity)}`}>
                      {selectedItem.rarity}
                    </Badge>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  {selectedItem.tokenId && (
                    <div>
                      ...
                      <p className="text-sm font-mono bg-black/30 p-2 rounded">{selectedItem.tokenId}</p>
                    </div>
                  )}

                  {selectedItem.mintedAt && (
                    <div>
                      <h4 className="text-sm font-medium text-gray-400 mb-1">สร้างเมื่อ</h4>
                      <p className="text-sm">{new Date(selectedItem.mintedAt).toLocaleString()}</p>
                    </div>
                  )}
                </div>
              ...
              <Button variant="ghost" className="w-full" onClick={handleCloseDetails}>
                กลับไปยังรายการไอเทม
              </Button>
            </motion.div>
          ) : (
            <motion.div key="item-list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="mb-4">
              ...
              {gameData.inventory.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-gray-400 max-w-md">ต่อสู้กับมอนสเตอร์เพื่อรับไอเทมและอาวุธที่สามารถเก็บเป็น NFT ได้</p>
                </div>
              ) : (
                <>
