...
export default function BattleArea({
  ...
  const generateMonster = () => {
    ...
    const monsterName = areaData.monsters[Math.floor(Math.random() * areaData.monsters.length)]
    const health =
      Math.floor(Math.random() * (areaData.healthRange[1] - areaData.healthRange[0] + 1)) + areaData.healthRange[0]
    const reward =
      Math.floor(Math.random() * (areaData.rewardRange[1] - areaData.rewardRange[0] + 1)) + areaData.rewardRange[0]
  ...
  const generateRandomItem = () => {
    ...
    const prefixes = ["แห่งพลัง", "แห่งความเร็ว", "แห่งปัญญา", "แห่งความอึด", "แห่งโชค"]
    ...
    switch (type) {
      case "อาวุธ":
        name = ["ดาบ", "ขวาน", "ค้อน", "หอก", "ธนู"][Math.floor(Math.random() * 5)]
        break
      case "เกราะ":
        name = ["เกราะอก", "หมวก", "ถุงมือ", "รองเท้า", "กางเกง"][Math.floor(Math.random() * 5)]
        break
      case "เครื่องประดับ":
        name = ["แหวน", "สร้อยคอ", "ต่างหู", "เข็มกลัด", "เข็มขัด"][Math.floor(Math.random() * 5)]
        break
      case "ยา":
        name = ["ยาฟื้นพลัง", "ยาเพิ่มพลัง", "ยาป้องกัน", "ยาพิษ", "ยาวิเศษ"][Math.floor(Math.random() * 5)]
        break
    }

    const fullName = `${name}${rarity === "ตำนาน" ? "แห่งตำนาน" : rarity === "หายาก" ? prefix : ""}`

    return {
      name: fullName,
      description: `${type}${rarity === "ตำนาน" ? "ระดับตำนาน" : ""} ที่เพิ่มพลัง${type === "อาวุธ" ? "โจมตี" : type === "เกราะ" ? "ป้องกัน" : type === "เครื่องประดับ" ? "พิเศษ" : "ฟื้นฟู"}`,
  ...
  const handleAttack = () => {
    ...
    // คำนวณความเสียหาย
    const baseDamage = (gameData.damage || 1) * (gameData.upgrades?.damageMultiplier || 1)
    ...
    const newHealthPercent = Math.max(0, monsterHealth - (damage / monster.maxHealth) * 100)
  ...
  useEffect(() => {
    return () => {
      if ((monstersDefeated > 0 || coinsEarned > 0) && gameData?.walletAddress) {
        recordBattle(gameData.walletAddress, {
          monstersDefeated,
          coinsEarned,
          itemsFound: 0,
        }).catch((err) => console.error("Error recording battle on unmount:", err))
      }
    }
  ...
  return (
    <div className="relative p-6" ref={battleAreaRef}>
      {/* พื้นหลังพื้นที่ */}
      <div className={`absolute inset-0 ${monster.background} rounded-xl opacity-70`}></div>
          ...
          <AnimatePresence>
            {damageNumbers.map((damage) => (
              <motion.div
                ...
                animate={{ opacity: 0, y: -50, scale: damage.isCritical ? 1.5 : 1 }}
                exit={{ opacity: 0 }}
                transition={{ duration: 1 }}
                className={`absolute z-20 font-bold ${damage.isCritical ? "text-red-500 text-2xl" : "text-yellow-300 text-xl"}`}
        ...
        {showTreasure && treasureItem && (
          ...
          >
            <div className="bg-gradient-to-b from-amber-900/80 to-yellow-900/80 p-6 rounded-xl border-2 border-yellow-500/50 max-w-md w-full">
              <h3 className="text-xl font-bold text-center mb-4 text-yellow-300">พบสมบัติ!</h3>
              ...
              <div className="flex flex-col items-center mb-6">
                ...
                <p className="text-sm text-gray-300 text-center mb-2">{treasureItem.description}</p>
              ...
              <div className="flex justify-center">
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  ...
                  >
                    ...
                    {treasureItem.rarity === "ตำนาน" ? "สร้าง NFT" : "เก็บไอเทม"}
                  </Button>
                </motion.div>
              </div>
            </div>
