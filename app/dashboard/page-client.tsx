...
interface GameData {
  ...
  items?: any[];
  lastUpdated?: string;
  [key: string]: any; // รองรับฟิลด์เพิ่มเติมที่อาจมีในอนาคต
}

export default function DashboardClient({ initialGameData = null }: { initialGameData: GameData | null }) {
  const router = useRouter();
  const { address, isConnected, registerPlayer } = useWeb3();
  ...
  const checkBlockchainRegistration = async (walletAddress: string) => {
    // ป้องกันการเรียกซ้ำ
    if (isCheckingRegistration || hasCheckedRef.current) return true;
    
    try {
      setIsCheckingRegistration(true);
      const response = await fetch(`/api/web3/register-check?address=${walletAddress}`);
      if (response.ok) {
  ...
  useEffect(() => {
    ...
    if (address && addressFromCookie !== address) {
      console.log("Cookie address doesn't match wallet address, updating cookie");
      // อัพเดต cookie ให้ตรงกับ address จาก wallet
      document.cookie = `player_address=${address}; path=/; max-age=${60 * 60 * 24 * 30}; SameSite=Lax`;
    }
  ...
  }, [address, router, initialGameData, registerPlayer]);
  ...
  if (isBlockchainRegistered === false) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen p-4">
        ...
        <p className="text-lg mb-6">คุณจำเป็นต้องลงทะเบียนบนบล็อกเชนก่อนเริ่มเล่นเกม</p>
        <button
  ...
  if (!gameData || !playerAddress) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen p-4">
        ...
        <p className="text-lg mb-6">กรุณาเชื่อมต่อกระเป๋าเงินและลองใหม่อีกครั้ง</p>
        <button
          onClick={() => router.push("/")}
          className="px-6 py-3 bg-blue-600 rounded-lg font-medium hover:bg-blue-700 transition-colors"
        >
  ...
  return <GameDashboard playerAddress={playerAddress} initialGameData={gameData} />;
} 